...
int main(int argc,char **argv)
{
  ...
  if (activeclients[i].child == 0) {
    ...
    /* $CURVECPCLIENTPUBKEY -- d3v11 */
    unsigned char CURVECPCLIENTPUBKEY[64] = {0};
    int I, L = 0;
    for(I=0;I<32;++I){
      CURVECPCLIENTPUBKEY[L] = clientlongtermpk[I] >> 4;
      if (CURVECPCLIENTPUBKEY[L] < 10) CURVECPCLIENTPUBKEY[L] += 48;
      else CURVECPCLIENTPUBKEY[L] += 87;
      ++L;
      CURVECPCLIENTPUBKEY[L] = clientlongtermpk[I] % 16;
      if (CURVECPCLIENTPUBKEY[L] < 10) CURVECPCLIENTPUBKEY[L] += 48;
      else CURVECPCLIENTPUBKEY[L] += 87;
      ++L;
    }
    setenv("CURVECPCLIENTPUBKEY",CURVECPCLIENTPUBKEY,1);
