...
re_SERVER_PRIVMSG_NOTICE_TOPIC = re.compile('^:['+RE+']+![~:#'+RE+'.]+@[:#'+RE+'.]{1,256} ((PRIVMSG)|(NOTICE)|(TOPIC)) [#&!+]['+RE+']+ :.*$',re.IGNORECASE).search
 ...
 if pipe_revents(0):
  ...
  if re_SERVER_PRIVMSG_NOTICE_TOPIC(buffer):
   ...
   if dst in channels:
    ...
    else: src = src.split('!',1)[0] + '!*@' + hexlify(crypto_hash_sha256(src)[:4])
    ...
    src = src.replace('!*@','@',1) + '> '
    if COLOUR and buffer.split('!',1)[1].split('@')[0] == 'sign':
     src = '\x03' + str(ord(hexlify(crypto_hash_sha256(src)[0])[:2].decode('hex'))%16) + src.replace('@','\x0f@',1)
    if cmd == 'TOPIC':
